...
export function useEnterMatchScoreController({
  ...
  const isPrimaryGenerateDisabled =
    isGenerating || isQrSessionBusy || !hasValidationLink;
  ...
  const onGenerateQr = useCallback(async (options?: { showSuccessToast?: boolean }) => {
    if (!canGenerateQr) {
  ...
  useEffect(() => {
    ...
    const timeoutId = window.setTimeout(() => {
      lastAutoGeneratedQrKeyRef.current = currentQrRequestKey;
