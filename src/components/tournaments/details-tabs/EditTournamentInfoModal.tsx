...
export function EditTournamentInfoModal({ open, onOpenChange, tournament }: EditTournamentInfoModalProps) {
  ...
  const initialClubId = tournament.club?.id ?? "";
  const initialSponsorId = tournament.sponsor?.id ?? "";
  const [selectedClubId, setSelectedClubId] = useState(initialClubId);
  const [selectedSponsorId, setSelectedSponsorId] = useState(initialSponsorId);

  const clubs = adminClubsData?.clubs ?? [];
  const clubsWithFallback = (() => {
    if (!initialClubId) return clubs;
    const exists = clubs.some((club) => club.id === initialClubId);
    if (exists) return clubs;
    return [{ id: initialClubId, name: tournament.club?.name ?? t("tournaments.unknownClub"), courtCount: 0 }, ...clubs];
  })();

  const { data: sponsorsData } = useClubSponsors(selectedClubId || null);
  const sponsors = sponsorsData?.sponsors ?? [];
  const activeSponsors = sponsors.filter((sponsor) => sponsor.status === "active");

  const hasChanges = selectedClubId !== initialClubId || selectedSponsorId !== initialSponsorId;
    ...
    try {
      ...
      const message =
        error && typeof error === "object" && "response" in error
          ...
          : null;
      toast.error(message ?? t("tournaments.saveError"));
    }
  };
